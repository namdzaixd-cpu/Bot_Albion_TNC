...
new_run_guildcheck = r"""    async def run_guildcheck(self, guild: discord.Guild):
...
content = re.sub(r'    async def run_guildcheck.*?return removed, None', lambda m: new_run_guildcheck, content, flags=re.DOTALL)
...
new_guildconfig = r"""    @app_commands.command(name="guildconfig", description="Cấu hình Guild ID, log channel, officer role & member role cho GuildCheck (Officer only)")
...
content = re.sub(r'    @app_commands\.command\(name="guildconfig".*?ephemeral=True\)', lambda m: new_guildconfig, content, flags=re.DOTALL)
